...
function bodyParagraphs(kind: 'leon' | 'group', prospect: OutreachProspect): string[] {
  ...
  if (kind === 'group') {
    ...
    return [
      ...
      `Le preparé una auditoría de ${name}.${ratingLine} El grupo ${name} opera varias sucursales; con RateTap puede verlas todas en un solo panel, medir sucursal por sucursal y captar reseñas privadas antes de que lleguen a Google. ${auditLine}`,
...
function touch2Paragraphs(kind: 'leon' | 'group', prospect: OutreachProspect): string[] {
  ...
  return [
    ...
    `Cada reseña de 1 estrella que aterrice públicamente en Google le cuesta clientes reales. RateTap la intercepta antes de que se publique y le da una segunda oportunidad de atenderla en privado.`,
  ];
}
